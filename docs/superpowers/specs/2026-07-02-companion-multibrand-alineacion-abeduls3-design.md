...
- **SA1 — Almacenamiento S3 de edo → el Companion firma con el `id` canónico (NO `edoId`).** La evidencia real (captura de `HandlePhotos.asp`: `entourage-uploads.s3.amazonaws.com/original/1004/2025/11/16/.../UPID_{orderId}/archivo.png`) muestra que el S3 de edo se indexa por el **`id` canónico** (`1004`), no por `edoId` (`854569`, que es solo el `member_id` de *listado*). **Decisión:** el Companion presigna él mismo, replicando el esquema real de edonext `original/{id canónico}/{yyyy}/{mm}/{dd}/{timestamp}/UPID_{orderId}/{filename}` en el bucket de edo (`entourage-uploads`). **El `edoId` deja de usarse para keys.** ⚠️ **Verificación externa pendiente:** el segmento `UPID_{orderId}` no está documentado en abeduls3 — confirmar con el equipo edo de dónde sale `orderId` (metadata del cliente) y si es obligatorio. El `assets.s3Prefix='brands/edo/'` del registro de abeduls3 **no** refleja el S3 real de edonext y **no se usa** para edo (ver D6).
...
- **Esquema de key = el real de edonext:** `original/{id}/{YYYY}/{M}/{D}/{timestamp}/UPID_{orderId}/{filename}`. El `orderId` viene de la metadata del upload (cliente). ⚠️ **`UPID_{orderId}` a confirmar con edo** (SA1): si `orderId` no está disponible, el segmento se omite o se confirma su origen antes de escribir a producción.
...
- **Verificación externa pendiente (SA1):** esquema completo (`UPID_{orderId}`) y bucket con el equipo edo — cubierto por el smoke test (Task 7.2) antes de escribir a prod.
...
4. `requireAuth` garantiza `req.user`. Key-builder (SA1, **id canónico**): `original/1004/2026/7/2/<ts>/UPID_{orderId}/<file>` en bucket `entourage-uploads`.
...
- **Unit:** `identity` (merge, prototype-pollution, tipos, longitud, host allowlist — portar `packages/brands/tests/identity.test.ts`); `detect` (exact-match, `BRAND_FORCE`, 404 en host desconocido); `session-resolver` (200/401/3xx/status0/4xx/5xx/timeout/16KB, breaker order, cache full-user hit, **enrichEdoUser puebla `edoId`**); `whoami-breaker` (3→open, half-open, clear on success, concurrencia); `s3.key-builder` (**id canónico** para todas las marcas, esquema edo `original/{id}/{fecha}/{ts}/UPID_{orderId}/{file}`, **401 si falta `user.id`**); `companion.factory` (allowLocalUrls por entorno, `validHosts`, uploadUrls derivado).
...
| Riesgo | Mitigación |
|---|---|
| Esquema S3 de edo incompleto (`UPID_{orderId}`, SA1) | `id` canónico ya verificado en captura real; smoke test + confirmación del segmento `UPID` con edo antes de escribir a prod |
