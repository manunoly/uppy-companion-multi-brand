...
- `src/modules/brand/`
  ...
  - `registry.ts` — the code-only, deep-frozen **base registry**, one entry per known slug. `getServableSlugs()` returns only slugs with a non-empty `companionHosts` (today: `edo` and `abe`; `picaboo` is registered but not servable — no external whoami endpoint confirmed yet, see the registry's own comments).
...
- `src/modules/auth/`
  - **Contract for the `abe` brand: [`docs/contracts/abe-session-auth.md`](docs/contracts/abe-session-auth.md).** abe runs on Better Auth's two-cookie model (`session_token` = the credential, `session_data` = a 300 s signed JWT cache verifiable offline against the auth origin's JWKS). Read it before touching `session-resolver.ts` for abe: the six-state result, the mandatory credential binding, the `poisoned` vs `unavailable` split, and the ~5 min 45 s revocation window (300 s `session_data` + 45 s `CACHE_TTL_SECONDS`) accepted in §3.3.
  - `session-resolver.ts` — `resolveSession(brand, cookieHeader)`, two branches over one shared tail (mirrors abeduls3's `resolvePartnerSocketIdentity.ts`). **Order is a security property.** Branch on `brand.auth.kind`: `capsule` (abe) reads the Better Auth `session_token` under both derived namespaces (`abe-cookie-names.ts`), `partner-whoami` reads the registry's `sessionCookieName`. Then, in both: credential absent -> `unauthenticated` (this MUST precede the SSRF gate, or an anonymous request against a brand with a broken `whoamiUrl` answers 403 instead of 401) -> `resolveValidatedWhoamiTarget` (SSRF gate; `misconfigured` never reaches `fetch`) -> build the forwarded header via `buildCookieHeader` (a malformed value is `unauthenticated` and must precede the breaker check, or an unauthenticated caller could open the breaker for every user of the brand). **capsule only, next:** `getAbeSessionVerifier(brand).verify()` — `valid` returns with no Redis, no fetch and no breaker; `unavailable` logs a warning and falls through **without touching the breaker**; `credential-mismatch` falls through forwarding the credential **alone**; `absent`/`expired`/`poisoned` fall through with both cookies (chunk-aware, `better-auth-cookies.ts`). Shared tail: Redis cache read (`companion-whoami:{slug}:{sha256(credential)}`, 45s TTL, full serialized `BrandUser`) **before** the breaker gate, so a hot identity survives a partner blip -> circuit breaker `isOpen` -> `fetch(whoamiUrl, { redirect: 'manual', signal: AbortSignal.timeout(5000) })` -> status interpretation (any redirect form or `status===0` -> `unavailable` + `recordFailure`; `401`/`403` -> `unauthenticated` + `recordSuccess`, the upstream answered about this session; every other 4xx -> `unavailable` + `recordFailure` + a warning, because `recordSuccess` would clear the brake a `429` just asked for; 5xx -> `unavailable` + `recordFailure`) -> 16KB streaming body cap -> `normalizeBrandUser` -> (`slug==='edo'` only) `enrichEdoUser`.
  - `src/vendor/auth-verify/` — a **byte-identical** vendored copy of abeduls3's `packages/auth-verify/src/`. Never edit it; a hash manifest test fails if you do. Fix upstream and re-sync — see [`src/vendor/auth-verify/VENDORED.md`](src/vendor/auth-verify/VENDORED.md).
...
- `src/modules/folders/folders.service.ts` — `fetchFolders(forwardCookie, brand)` reads `brand.public?.foldersUrl` (optional) and relays the header `resolveSession` already decided on — never a rebuilt one; missing URL, non-ok response, or any exception all degrade silently to `[]` with a `logger.warn` (SA3 — kept for future Dropbox/GoogleDrivePicker use even though today's brands don't call it from the designer).
- `src/lib/`
...
2. **`<SLUG>_BRAND_OVERRIDE`** env var (JSON) — merged in by `identity.ts#resolveEffectiveAuth`. Only the **string fields of `auth`** may be overridden: `whoamiUrl`, `signInUrl`, `signOutUrl`, `sessionCookieName`, `authIssuer` (gated by the code-only `authAllowedHosts`). Every rejected field (protected key, prototype-pollution attempt, wrong type, invalid URL/cookie-name shape) is logged via `logger.warn({slug, field})` and silently dropped — the brand falls back to the base value for that field, never throws.
...
- **A non-servable brand (`companionHosts: []`, `picaboo` today) never gets a Companion instance, secrets, or a route.** Don't invent a `whoamiUrl` for one to "make it work" — see the registry's own comments for why (no confirmed external endpoint). Setting `BRAND_FORCE` to one throws at boot (`assertBrandForceIsServable`).
