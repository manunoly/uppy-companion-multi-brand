...
/**
 * Zod schemas for the new brand contract (`brand.contract.ts`). Kept as
 * `.next.ts` per Task 2.6 of the abeduls3-alignment plan — `brand.schema.ts`
 * is still imported by the legacy `config/env.ts` today, so rewriting it here
 * would break `typecheck` before the atomic cutover (Task 2.7).
 *
